...
namespace OpenSimProfile.Modules.OpenProfile
{
    ...
    public class OpenSimProfile : IProfileModule, ISharedRegionModule
    {
        ...
        {
            if(m_log.IsDebugEnabled) m_log.DebugFormat( "{0}", MethodBase.GetCurrentMethod());
        ...
        private Hashtable GenericXMLRPCRequest(Hashtable ReqParams, string method, string server)
        {
            ...
            {
                m_log.ErrorFormat("Unable to connect to Profile " +
            ...
            {
                m_log.ErrorFormat(
                    "Unable to connect to Profile Server {0}. Method {1}, params {2}. " +
            ...
            if (money != null)
            {
                if (!money.AmountCovered(remoteClient.AgentId, queryclassifiedPrice))
                {
                    remoteClient.SendCreateGroupReply(UUID.Zero, false,
                        "Insufficient funds to create a classified ad.");
                    return;
                }
            }
        ...
        public void RequestAvatarProperties(IClientAPI remoteClient, UUID avatarID)
        {
            ...
            if (String.IsNullOrEmpty(avatarID.ToString()) || String.IsNullOrEmpty(remoteClient.AgentId.ToString()))
            {
                ...
                m_log.InfoFormat("[PROFILE]: This should not happen remoteClient.AgentId {0} - avatarID {1}",
                    remoteClient.AgentId, avatarID);
                return;
            }
        ...
        public void UpdateAvatarProperties(IClientAPI remoteClient, UserProfileProperties newProfile)
        {
            ...
            if (remoteClient.AgentId == newProfile.UserId)
            {
                ...
                ReqHash["UserId"] = remoteClient.AgentId.ToString();
                ReqHash["WebUrl"] = newProfile.WebUrl;
                ...
                RequestAvatarProperties(remoteClient, newProfile.UserId);
            }
        }
